if (!nexacro.ExtWebSocket) 
{
		nexacro.WebSocketEventInfo = function(obj, userid, flag, message, id) 
    {
        this.id = this.eventid = id || "onbroadcast";
        this.fromobject = this.fromreferenceobject = obj; // this property is not exist to menual
        this.userid = userid;
        this.flag = flag;
        this.message = message;
    };
    var _pWebSocketEventInfo = nexacro._createPrototype(nexacro.Event, nexacro.WebSocketEventInfo);
    nexacro.WebSocketEventInfo.prototype = _pWebSocketEventInfo;
    ...
    // nexacro.ExtCombo
    // ==============================================================================
    nexacro.ExtWebSocket = function(id, position, left, top, width, height, right, bottom, parent) 
    {
        ...
        this.itemopacity ="";
       	...
       	//this._event_list["onbroadcast"] = 1;
    ...
    var _pExtWebSocket = nexacro._createPrototype(nexacro.Static);
    nexacro.ExtWebSocket.prototype = _pExtWebSocket;
    _pExtWebSocket._type = "nexacroExtWebSocket";
    _pExtWebSocket._type_name = "ExtWebSocket";
    
    _pExtWebSocket.set_userprpo = function (v) 
    {
        this.userprop = v;
    }
    
    _pExtWebSocket.set_itemopacity = function (v) 
    {
        this.itemopacity = v;
    }
    ...
    _pExtWebSocket.wsstart = function ()
    {
       // check if WebSockets are supported by the browser
				...
				{
					alert("웹소켓을 사용할 수 없는 브라우저입니다.");						
				}
    }

		_pExtWebSocket.wsstop = function() {
		...
		_pExtWebSocket.logon = function() {
				...
				lWSC.logon( lURL, "guest", "guest", {
					...
					OnOpen: function( aEvent ) {
						_self.sendMessage( SYS, IN, "Connection to " + lJWSID + " established." );
					},
					// OnMessage callback
						...
						if( aToken ) {
							...
							if( aToken.type == "response" ) {
								...
								if( aToken.reqType == "login" ) {
									if( aToken.code == 0 ) {
										_self.sendMessage( SYS, IN, "Welcome '" + aToken.username + "', ID : "+lWSC.getId() );
										lWSC.getAuthClients({
											pool: null
										});
									} else {
										_self.sendMessage( SYS, IN, "Error logging in '" + eUsername.value + "': " + aToken.msg );
									}
								} else if( aToken.reqType == "getClients" ) {
									_self.sendMessage( SYS, IN, "Clients (" + aToken.count + "): " + aToken.clients );
								}
							} else if( aToken.type == "event" ) {
								
							} else if( aToken.type == "goodBye" ) {
									_self.sendMessage( SYS, IN, lJWSID + " says good bye (reason: " + aToken.reason + ")!" );
							} else if( aToken.type == "broadcast" ) {
								if( aToken.data ) {
									_self.sendMessage( aToken.sourceId, IN, aToken.data );
								}
							}
						}
					},
					...
					OnClose: function( aEvent ) {
						lWSC.stopKeepAlive();
						_self.sendMessage( SYS, IN, "Disconnected from " + lJWSID + "." );
					}
				});
			}
			
		_pExtWebSocket.wsbroadcast = function(eMessage) {
			var lMsg = eMessage;
			var _self = this;
			var lTarget = "*";
			if( lMsg.length > 0 ) {
				_self.sendMessage( USR, OUT, lMsg );
				var lRes;
				if( !lTarget || lTarget == "*") {
				...
				if( lRes.code != 0 ) {
					_self.sendMessage( SYS, OUT, "broadcast: " + lRes.msg );
				}
				// eMessage.value = "";
			}
		}
		...
		_pExtWebSocket.sendMessage = function( aUsername, aEvent, aString ) {
			var lFlag;
			switch( aEvent ) {
				case IN: lFlag = "<"; break;
			...
			//trace(aUsername + " " +	lFlag + " " +	aString);
						
			this.on_fire_broadcast(this, aUsername, lFlag, aString, "on broadcast");
			
			
			//this.on_broadcast(this, e);
			//this.on_fire_click(this, e);
		}
		

		_pExtWebSocket.on_fire_broadcast = function (obj, userid, flag, message, reason)
    {
    	//trace("on_fire_broadcast");
    	if (this.onbroadcast && this.onbroadcast._has_handlers)
       {
	    	var evt = new nexacro.WebSocketEventInfo(this, userid, flag, message, reason);
	      return this.onbroadcast._fireEvent(this, evt);
      }
        return true;
    };
/*
    _pExtWebSocket.on_fire_broadcast = function (objExtWebSocket, eEventInfo)
    {
    		trace(this);
    		trace(this.onlbuttondown);
        if (this.onbroadcast && this.onbroadcast._has_handlers)
        {
            return this.onbroadcast._fireEvent(this, eEventInfo);
        }
        return true;
    };
 */
   
    delete _pExtWebSocket;
