    ...
    if (!nexacro.Init_systembase_runtime)
    {
        ...
        nexacro._print = function(pThis, refform, defaultprint, valign, halign) 
        {
            ...
            if (elem && elem._handle)
            {
                ...
                nexacro.__print(elem._handle, form_elem._handle, defaultprint, valign, halign);
            }
        };
        ...
        {
            //html보다 runtime이 size가 약간 큼;
            //runtime에서는 linspace등을 감안하므로 차이가 생김;
    	    if (text && text.length > 0 && font && !font._isEmpty)
    	    {
    	        //getTextSize에서도 가공을 하여 중복기능이나 _getTextSize2를 호출하면 이곳에 바로 와서 wordwrap=true가 가공안됨;
    	        ...
    	        if (multiline)
    	        {
    	            ...
    	            {
    	                return nexacro.__getTextSize(text, font.face, font.size, font._antialias, font._bold, font._italic, font._underline, font._strikeout, wordwrap, content_width);
    	            }
    	            else
    	            {
    	                ...
    	                for (var i = 0; i < lcnt; i++)
    	                {
    	                    ...
    	                    var line_size = nexacro.__getTextSize(lines[i], font.face, font.size, font._antialias, font._bold, font._italic, font._underline, font._strikeout, "none");
    	        ...
    	        {
    	            return nexacro.__getTextSize(text.replace(/\r\n|\n|\r/, ''), font.face, font.size, font._antialias, font._bold, font._italic, font._underline, font._strikeout, "none");
    	        }
        ...
        {
            var item_value = nexacro.__findclickFileUploadHandle(comp_name, item_comp_name, handle, item_comp._multiselect);
            if (item_comp)
            {
        ...
        {
            var handle = nexacro.__createFileUploadHandle(name, iframe_id, callback_fn, pThis);
            if (handle)
       ...
       nexacro._create_filedownload_handle = function(callback_fn, pThis) 
            {
            var handle = nexacro.__createFileDownloadHandle(callback_fn, pThis);
            if (handle)
                pThis._handle = handle;
	    };
        ...
        nexacro._submit = function(form_id, action, handle, exp_info_data)
	    {
            application._beginCommProgress();
            var handle = nexacro.__submitFileUploadHandle(form_id, action, handle, exp_info_data);
            if (handle == null)
                application._endCommProgress();
	    };
	        ...
	        for (var i = 0; i < param_cnt; i++)
	        {
	            ...
	            if (varInfo)
	            {
	                ...
	                if (id == "ErrorCode")
	                {
	                    ...
	                    {
	                        this._error_info[0] = -1;
	                    }
	                    else
	                    {
	                        this._error_info[0] = code;
	                    }
	                }
	                else if (id == "ErrorMsg")
	                {
